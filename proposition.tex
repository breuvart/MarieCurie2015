...
In this project, we propose to investigate a way to recover as much information as possible from type inference which is one of the most complex kinds of compositional and static analysis.  We do not intend to stamp out global analyses that are proven more efficient in the general cases. The ideal outcome would be a static analyser for functional languages that do not fall too far behind existing ones, while being more scalable.
...
In addition to the host supervisor, the Theory group includ numerous strong figures in fields that are particuliarly relevent for us: 
\begin{itemize}
\item linear and dependent type systems (Krishnaswami, Escardo),
\item categorical semantics of programming languages  (Levy, Reddy),%with a strong understanding of monads
\item domain theory (Ahim Jung, Steve Vickers and Martin Escardo),
\item linear logic (Eike Ritter). %although he is more of a security researcher now. 
\end{itemize}
